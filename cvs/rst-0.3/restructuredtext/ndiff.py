...
def fancy_replace(a, alo, ahi, b, blo, bhi, charjunk=None):
    if TRACE:
    ...
    cruncher = SequenceMatcher(charjunk)
    ...
    fancy_helper(a, alo, best_i, b, blo, best_j, charjunk)
    ...
    fancy_helper(a, best_i+1, ahi, b, best_j+1, bhi, charjunk)

def fancy_helper(a, alo, ahi, b, blo, bhi, charjunk=None):
    if alo < ahi:
        if blo < bhi:
            fancy_replace(a, alo, ahi, b, blo, bhi, charjunk)
        else:
            dump('-', a, alo, ahi)
    elif blo < bhi:
        dump('+', b, blo, bhi)
...
def lcompare(a, b, linejunk=None, charjunk=None):
    cruncher = SequenceMatcher(linejunk, a, b)
...
def fcompare(f1name, f2name, linejunk=None, charjunk=None):
    f1 = fopen(f1name)
    f2 = fopen(f2name)
    if not f1 or not f2:
        return 0
    ...
    lcompare(list1, list2, linejunk, charjunk)
...
def main(args):
    ...
    if noisy:
        print '-:', f1name
        print '+:', f2name
    return fcompare(f1name, f2name, IS_LINE_JUNK, IS_CHARACTER_JUNK)
